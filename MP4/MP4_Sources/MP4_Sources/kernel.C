...
int main() {
   ...
   GDT::init();
    ...
    class PageFault_Handler : public ExceptionHandler {
      /* We derive the page fault handler from ExceptionHandler and overload the method handle_exception. */
      public:
      virtual void handle_exception(REGS * _regs) {
...
//#define _TEST_PAGE_TABLE_
    ...
    Console::puts("\n\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Making heap VMPool <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    VMPool heap_pool(256 MB, 256 MB, &process_mem_pool, &pt1);
    ...
    GenerateVMPoolMemoryReferences(&heap_pool, 75, 100);
